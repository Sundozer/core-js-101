 ...
 */
function getStringFromTemplate(v1, v2) {
  return `Hello, ${v1} ${v2}!`;
}
...
function extractNameFromTemplate(str) {
  return str.slice(str.search(' ') + 1, -1);
}
...
function removeFirstOccurrences(str, val) {
  return str.slice(0, str.search(val)) + str.slice(str.search(val) + val.length);
}
